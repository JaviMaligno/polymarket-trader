  ...
  try {
    const result = await pool.query(`
      ...
      WHERE market_type IN ('crypto_intraday', 'crypto_daily')
        AND question ~* '(airdrop|launch|mainnet|testnet|listing|points|campaign|fdv|fully diluted|market cap|governance|proposal|vote|partnership|token generation|tge|unlock|vesting)'
    ...
    console.log(`Scanned ${result.rows.length} crypto-event candidate markets`);
    console.log(`Found ${updates.length} markets that would be reclassified`);

    if (updates.length > 0) {
      console.table(
        updates.slice(0, 100).map((row) => ({
          id: row.id,
