...
describe('AutoSignalExecutor', () => {
  ...
  describe('Drawdown proximity check', () => {
    it('blocks new opens when equity drawdown exceeds CB_THRESHOLD - 1', async () => {
      const exec = new AutoSignalExecutor({ enabled: true, cooldownMs: 0 });
      (query as any).mockImplementation((sql: string) => {
        if (sql.includes('FROM markets WHERE id')) {
          return Promise.resolve({ rows: [{ is_active: true, is_resolved: false, end_date: null }] });
        ...
        if (sql.includes('FROM paper_account')) {
          // Capital $8550 with initial $10000 → equity drawdown 14.5% > 15-1=14%
          return Promise.resolve({ rows: [{ available_capital: '8450', current_capital: '8550' }] });
        }
        if (sql.includes('FROM paper_positions WHERE closed_at IS NULL')) {
          // No open positions → equity == capital → equity drawdown also 14.5%
    ...
    it('allows new opens when equity drawdown is below CB_THRESHOLD - 1 even if capital-only exceeds it', async () => {
      const exec = new AutoSignalExecutor({ enabled: true, cooldownMs: 0 });
      (query as any).mockImplementation((sql: string) => {
        if (sql.includes('FROM markets WHERE id')) {
          return Promise.resolve({ rows: [{ is_active: true, is_resolved: false, end_date: null }] });
        ...
        if (sql.includes('FROM paper_account')) {
          // Capital $8550: capital-only drawdown = 14.5% > 14% (would block without equity fix)
          return Promise.resolve({ rows: [{ available_capital: '8450', current_capital: '8550' }] });
        }
        if (sql.includes('FROM paper_positions WHERE closed_at IS NULL')) {
          // Open position worth $600: equity = $9150, equity drawdown = 8.5% < 14% → allow
          return Promise.resolve({ rows: [{ total_exposure: '600' }] });
        }
        if (sql.includes('market_score') && sql.includes('current_price_yes')) {
          return Promise.resolve({ rows: [{ market_score: '0.5', current_price_yes: '0.5', volume_24h: '1000', spread: '0.01', end_date: null }] });
        }
