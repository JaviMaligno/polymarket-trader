...
describe('FLBExecutor.executeCandidates', () => {
  ...
  it('locks stake plus fee on the orderbook fill path', async () => {
    routeReads({ initialCapital: 10000, locked: 0 });
    const exec = new FLBExecutor();
    const result = await exec.executeCandidates([{
      marketId: 'm4', marketType: 'event_short', yesPrice: 0.05, spread: 0.01,
      ttrHours: 96, noTokenId: 'noTok', endDate: '2026-09-01T00:00:00Z',
      bookExecuted: true, bookExecutedNoPrice: 0.953, bookFee: 5,
    }], getFLBConfig());
    ...
    const lock = queryMock.mock.calls.find(c =>
      String(c[0]).startsWith('UPDATE paper_account') && String(c[0]).includes('flb_locked_capital'));
    expect(lock).toBeTruthy();
    expect(Number((lock as any[])[1][0])).toBeCloseTo(26, 6); // noStake 21 + fee 5
  });

  it('treats a lost insert race as a rejection without locking capital', async () => {
    queryMock.mockImplementation(async (sql: string) => {
      if (sql.includes('FROM paper_account') && sql.startsWith('SELECT')) {
        return { rows: [{ initial_capital: '10000', flb_locked_capital: '0' }] };
      }
      if (sql.includes("status = 'open'") && sql.includes('market_id')) return { rows: [] };
      if (sql.includes('GROUP BY') && sql.includes('end_date')) return { rows: [] };
      if (sql.startsWith('INSERT INTO flb_positions')) return { rowCount: 0, rows: [] }; // lost race
      if (sql.startsWith('UPDATE paper_account')) return { rowCount: 1, rows: [] };
      return { rows: [] };
    });
    const exec = new FLBExecutor();
    const result = await exec.executeCandidates([{
      marketId: 'm5', marketType: 'event_short', yesPrice: 0.05, spread: 0.01,
      ttrHours: 96, noTokenId: 'noTok', endDate: '2026-09-01T00:00:00Z',
    }], getFLBConfig());

    expect(result.rejected).toBe(1);
    expect(result.opened).toBe(0);
    expect(queryMock.mock.calls.some(c => String(c[0]).startsWith('UPDATE paper_account'))).toBe(false);
  });
});
